...
export function CompactKPIDashboard({
  ...
  const row2KPIs: KPIConfig[] = [
    {
      ...
      title: '满期边际贡献额',
      unit: '万',
      formatter: val => (val !== null && !isNaN(val) ? formatCurrency(val) : '-'),
  ...
  const row3KPIs: KPIConfig[] = [
    ...
    },
    {
      ...
      title: '保单件数(件)',
      unit: '',
      formatter: val => (val !== null && !isNaN(val) ? val.toLocaleString('zh-CN') : '-'),
      getColor: () => 'text-slate-700',
      decimals: 0,
    },
  ]
  ...
  const row4KPIs: KPIConfig[] = [
    ...
    },
    {
      ...
      title: '单均保费(元)',
      unit: '',
      formatter: val => (val !== null && !isNaN(val) ? Math.round(val).toLocaleString('zh-CN') : '-'),
      getColor: () => 'text-slate-700',
      decimals: 0,
    },
    {
